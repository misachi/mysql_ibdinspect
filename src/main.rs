...
use crate::rem0rec::{REC_N_NEW_EXTRA_BYTES, rec_get_next_offs};
use crate::ut::{
...
fn read_page(page_number: u32, args: &Args, buf: &mut [u8]) {
...
fn read_page_and_return_level(buf: &mut [u8], page_number: u32) -> u64 {
    let args = Args::parse();
    read_page(page_number, &args, buf);
...
fn reach_to_leftmost_leaf_level(buf: &mut [u8], root_page_number: u32) -> ERR {
    let mut page_level = read_page_and_return_level(buf, root_page_number);
    ...
    while page_level != 0 && page_level != u64::MAX {
        ...
        page_level = read_page_and_return_level(buf, child_page_number);
...
fn get_first_user_rec(page: &[u8]) -> u32 {
    let next_rec_offset = rec_get_next_offs(page, PAGE_NEW_INFIMUM, page_is_comp(page) as u32);
    PAGE_NEW_INFIMUM + next_rec_offset
}

fn get_next_rec(page: &[u8], rec: u32) -> u32 {
    ...
    if get_rec_type(&page, next_rec) == REC_STATUS_SUPREMUM as u8 {
        ...
        read_page(next_page_num, &args, buf);
...
fn copy_uncompressed_blob(
    ...
    while next_page_num != FIL_NULL {
        read_page(next_page_num, &args, page_buf);
...
fn copy_compressed_blob(
    ...
    loop {
        read_page(page_num, &args, page_buf);
...
fn print_all_recs_in_sdi_leaf_pages(page: &mut [u8], root_page_number: u32) {
    match reach_to_leftmost_leaf_level(page, root_page_number) {
        ERR::SUCCESS => {
            let mut rec = get_first_user_rec(page);
            let mut sdi_id: u64;
            let mut sdi_type: u64;

            while rec != 0 && get_rec_type(&page, rec) != REC_STATUS_SUPREMUM as u8 {
                sdi_type = mach_read_from_4(&page[(rec + REC_OFF_DATA_TYPE) as usize..]) as u64;
                ...
                if is_rec_data_external {
                    ...
                    let fsp_flags = page_header_get_field(buf, FSP_SPACE_FLAGS);
                 ...
                 */
                let mut uncompressed_sdi_buf = vec![0u8; 2 * sdi_uncomp_len as usize];
                ...
                println!(
                    "\nSDI Record - Type: {}, ID: {}, Uncompressed Length: {}, Compressed Length: {}",
                    sdi_type, sdi_id, sdi_uncomp_len, sdi_comp_len
                );
                println!(
                    "SDI Data: {}",
                    String::from_utf8_lossy(&uncompressed_sdi_buf)
                );
...
fn main() {
    ...
    if page_type == FIL_PAGE_SDI {
        ...
        print_all_recs_in_sdi_leaf_pages(buf, sdi_root_page_number);
    }
